...
// the code isn't run until the browser has finished rendering all the elements
  ...
  for (i = 0; i < workingHours.length; i++) {
    ...
    // var textOutputEl = $("<div>");
    // textOutputEl.attr("class", "col-8 col-md-10 description");
    // textOutputEl.attr("rows", "3");
    // textOutputEl.attr("id", "textOutput-" + (i + 1));
    // textOutputEl.number = i + 1;
  
    var textInputEl = $("<textarea>");
    textInputEl.attr("class", "col-8 col-md-10 description");
    textInputEl.number = i + 1;
    // textOutputEl.attr("id", "textInput-" + (i + 1));
  
    var saveBtn = $("<button>");
    saveBtn.text("Save");
    saveBtn.attr("class", "btn saveBtn col-2 col-md-1");
    saveBtn.attr('aria-label', "save");
    saveBtn.attr("id", "sButton-" + (i + 1));
    saveBtn.number = i + 1;
    ...
    // hourBlockEl.append(textOutputEl);
    // textOutputEl.append(textInputEl);
